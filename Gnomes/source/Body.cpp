...
bool Body::StartDragging(const CIwFVec2& position) {
	if (!m_bCanDrag) {
		return false;
	}
	if (!HitTest(position)) {
		// don't allow pinning outside the body
		return false;
	}
	...
	b2MouseJointDef def;
	def.target = b2Vec2(position.x, position.y); // pin at actual touch position
	def.bodyA = &m_xWorld.GetAnchorBody();
	def.bodyB = m_pxBody;
	def.maxForce = 300.0f * m_pxBody->GetMass();
...
void Body::MoveDragging(const CIwFVec2& position) {
	if (m_pxDragHandle) {
		m_pxDragHandle->SetTarget(b2Vec2(position.x, position.y));
	}
}
