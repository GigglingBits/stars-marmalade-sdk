...
LevelCompletion::LevelCompletion(LevelCompletionInfo& info) : 
	Page(),
...
bool LevelCompletion::IsCompleted(LevelCompletionInfo& info) {
	return info.GetKilledCount() < 1;
}

std::string LevelCompletion::GenerateCompletionText(LevelCompletionInfo& info) {
	std::ostringstream oss;

	oss << "Rescued: " << info.GetRescuedCount() << " / Lost: " << info.GetKilledCount() << std::endl;
