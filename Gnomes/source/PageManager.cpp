...
Page* PageManager::CreateNextPage(Page* oldpage) {
	...
	if (dynamic_cast<Preamble*>(oldpage)) {
		nextpage = FactoryManager::GetLevelFactory().Create(GetLevelId(m_iInititedLevelId));

	} else if (Level* level = dynamic_cast<Level*>(oldpage)) {
		LevelCompletionInfo info;
		if (level->GetCompletionState(info)) {
...
void PageManager::RestartLevel() {
	IW_CALLSTACK_SELF;
	SetNextPage(FactoryManager::GetPreambleFactory().Create(GetLevelId(m_iInititedLevelId)));
}
