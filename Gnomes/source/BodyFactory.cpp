...
Body* BodyFactory::CreateInstance(const BodyTemplate& conf) {
	...
	if (!copyconf.GetType().compare("hero")) {
		p = new Hero(instanceid, *bodydef, fixturedef, texturedef);
	} else if (!copyconf.GetType().compare("monster")) {
		p = new Monster(instanceid, *bodydef, fixturedef, texturedef);
	} else if (!copyconf.GetType().compare("target")) {
		p = new Target(instanceid, *bodydef, fixturedef, texturedef);
	} else if (!copyconf.GetType().compare("catapult")) {
		p = new Catapult(instanceid, *bodydef, fixturedef, texturedef);
