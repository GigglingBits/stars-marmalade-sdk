...
: LocalEffect(id, shapedef, texturedef), m_uiColour(0xffffffff), m_eFont(Renderer::eFontTypeNormal) {
}
...
void SplashText::SetText(std::string text, uint32 colour, Renderer::FontType font) {
	m_sText = text;
	m_uiColour = colour;
	m_eFont = font;
}
...
void SplashText::OnRender(Renderer& renderer, const FrameData& frame) {
	...
	renderer.DrawText(m_sText, GetPosition(), m_eFont, m_uiColour);
}
